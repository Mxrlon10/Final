...
#tabla prenatal_emb
	...
	function __construct($id,$nombre,$direccion,$telefono,$edad,$estado,$id_consultorio,
	                    #tabla prenatal
	...
	function crear(){
		...
		mysql_query("INSERT INTO pacientes (nombre, direccion, telefono, edad, sexo, email, estado, consultorio, control) 
								  VALUES ('$nombre','$direccion','$telefono','$edad','f','','$estado','$id_consultorio','prenatal')");
		
		$cans=mysql_query("SELECT MAX(id) AS id FROM pacientes");
			if($dat=mysql_fetch_array($cans))
			$id_paciente =$dat['id'];
			{
				$xSQL="INSERT INTO prenatal (id_paciente, nombre, opciones, familiares, personales, ultimo, parto, estado, consultorio) 
					               VALUES ('$id_paciente','$nombre','$opciones','$familiares','$personales','$ultimo','$parto','$estado','$id_consultorio')";
				mysql_query($xSQL);
				
				$preSQL="INSERT INTO prenatal_emb (id_paciente, pesopre, tallapre, fur, app, a, hemoglobina, rh, toxoplasmosis, glucosa, igm, 
													igg, vdrl, ego, hiv, egh, citologoa, otrospre, vacinacion, estado, consultorio) 
						                  VALUES ('$id_paciente','$pesopre','$tallapre','$fur','$app','$a','$hemoglobina','$rh','$toxoplasmosis','$glucosa','$igm',
